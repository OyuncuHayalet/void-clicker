...
void ImGui::Togglebox(const char* str_id, bool* v)
{
    ImVec2 p = ImGui::GetCursorScreenPos();
    ImDrawList* draw_list = ImGui::GetWindowDrawList();
    float height = ImGui::GetFrameHeight();
    float width = height * 1.80f;
    float radius = height * 0.0f;
    ...
    if (ImGui::IsItemHovered())
        col_bg = *v ? GetColorU32(ImGuiCol_CheckMark) : IM_COL32(35, 35, 35, 200);
    else
        col_bg = *v ? GetColorU32(ImGuiCol_CheckMark) : IM_COL32(35, 35, 35, 200);

    draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), col_bg, height * 0.5f);
    draw_list->AddCircleFilled(ImVec2(*v ? (p.x + width - height * 0.50f) : (p.x + height * 0.50f), p.y + height * 0.50f), height * 0.5f, IM_COL32(220, 220, 220, 244));
}

bool ImGui::Spinner(const char* label, float slow, float radius, int thickness) 
{
    ...
    int num_segments = 30;
    int start = abs(ImSin(g.Time * slow) * (num_segments - 5));
    ...
    window->DrawList->PathStroke(GetColorU32(ImGuiCol_Button), false, thickness);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 8.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (*((float*)p_data) > *((float*)p_max)) {
        memcpy(p_data, p_max, sizeof(void*));
    }
    else if (*((float*)p_data) < *((float*)p_min)) {
        memcpy(p_data, p_min, sizeof(void*));
    }
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, style.FramePadding.y - 1.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), ImColor(15, 15, 15), true, style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max + ImVec2(2, 2), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        window->DrawList->AddCircleFilled(grab_bb.Max - ImVec2(0, 0), 6, ImColor(205, 205, 205));
    }
    ...
    char value_buf2[64];
    const char* value_buf_end2 = value_buf2 + DataTypeFormatString(value_buf2, IM_ARRAYSIZE(value_buf2), data_type, p_min, format);

    char value_buf3[64];
    const char* value_buf_end3 = value_buf3 + DataTypeFormatString(value_buf3, IM_ARRAYSIZE(value_buf3), data_type, p_max, format);

    std::string value_buf_str(value_buf);
    std::string value_buf2_str(value_buf2);
    std::string value_buf3_str(value_buf3);

    float value_buf_float = std::atof(value_buf_str.c_str());
    float value_buf2_float = std::atof(value_buf2_str.c_str());
    float value_buf3_float = std::atof(value_buf3_str.c_str());

    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(((value_buf_float - value_buf2_float) / (value_buf3_float - value_buf2_float)), 0.5f), NULL);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 20, frame_bb.Min.y + style.FramePadding.y - 12), value_buf);
