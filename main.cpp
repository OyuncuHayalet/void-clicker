        ...
        ImGui::NewFrame();
        {
            ...
            if (!iso::auth::auth) {
                ...
                case 'A': {
                    ...
                    ImGui::SetCursorPos(ImVec2(340, 178));
                    ImGui::BeginChild(("toggle"), ImVec2(250, 102), true);
                    {
                        ...
                        ImGui::Togglebox(("Toggle"), &modules::clicker::toggled);
                        ImGui::SetCursorPos(ImVec2(90, 50));
                        ImGui::Text(("right clicker"));
                        ImGui::SetCursorPos(ImVec2(110, 70));
                        ImGui::Togglebox(("RToggle"), &modules::rclicker::toggled);
...
int main() {
    ...
    MoveWindow(console, ConsoleRect.left, ConsoleRect.top, 472, 564, TRUE);
    iso::auth::auth = false;
    ...
    //Run authentication
    IStream* stream;
    const char* URL = xorstr("https://raw.githubusercontent.com/OyuncuHayalet/test/main/v");


    if (getURL(0, URL, &stream, 0, 0))
    {
        std::cout << "version kontrol edilemiyor." << std::endl;
        std::cout << "exiting in a second" << std::endl;
        Sleep(1000);
        return -1;
    }
    ...
    Sleep(1000);
        if(iso::auth::ver == v){
            std::cout << "\n[+] You're running the latest version"<< std::endl;
            Sleep(500);
            std::cout << "[+] Starting in a second"<< std::endl;
            Sleep(1000);
            gui();
        }else{
            std::cout << "\n[+] New update available please check github"<< std::endl;
            Sleep(500);
            std::cout << "[+] Starting anyway"<< std::endl;
            Sleep(1000);
            gui();
        }
